...
impl EventStore {
    ...
    pub fn load_all_events(&self, from_global_id: i64) -> Result<Vec<StoredEvent>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare_cached(
            "SELECT id, stream_id, event_type, payload, timestamp, sequence
             FROM events
...
mod tests {
    ...
    fn test_create_and_read_events() {
        ...
        let read = store.load_stream_events("thread:abc", 0).unwrap();
    ...
    fn test_read_from_sequence() {
        ...
        let read = store.load_stream_events("stream:1", 2).unwrap();
        assert_eq!(read.len(), 1);
        assert_eq!(read[0].event_type, "E3");
    }

    #[test]
    fn test_load_all_across_streams() {
        let store = EventStore::in_memory().unwrap();
        ...
        let all = store.load_all_events(0).unwrap();
        assert_eq!(all.len(), 2);
    }
    ...
    fn test_stream_ids() {
        ...
        store
            .append_events("stream:b", &[("E1".to_string(), json!({}))])
            .unwrap();
        store
            .append_events("stream:a", &[("E2".to_string(), json!({}))])
            .unwrap();

        let ids = store.stream_ids().unwrap();
